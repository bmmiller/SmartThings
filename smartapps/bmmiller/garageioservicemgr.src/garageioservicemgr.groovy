...
def getGarageioDoors() {
    ...
    try 
    {
    	httpGet("${get_URI_ROOT()}/SyncController.php?auth_token=" + state.token + "&user_id=" + state.userid)
        	{ response ->            
...
def getAuthToken() {
    ...
    httpPost(params) {response -> 
    	state.token = response.data.data[0].authentication_token
...
def pollChildren() { 
    ...
    def args = "auth_token=" + state.token + "&user_id=" + state.userid   
